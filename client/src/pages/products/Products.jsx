...
import { Button, Form, Input, Modal, Select, Table, message, Row, Col, Switch, Dropdown, Menu } from 'antd';
...
const Products = () => {
  ...
  const [addedSellers, setAddedSellers] = useState([]); // Track globally added sellers
...
const { categories } = useCategories(); // Fetch categories from context
  ...
  // Load addedSellers from local storage on initial render
  useEffect(() => {
    const savedSellers = JSON.parse(localStorage.getItem('addedSellers')) || [];
    setAddedSellers(savedSellers);
  }, []);
...
const filterMenu = (
  ...
  const handleEditProduct = (record) => {
    ...
    if (record.seller) {
      ...
      setIsExistingUserSelected(false); // Reset when editing a product
    }
  ...
  const areMandatoryFieldsFilled = () => {
    const sellerValues = sellerForm.getFieldsValue();
    return (
      sellerValues.name &&
      sellerValues.phone &&
      sellerValues.address &&
      sellerValues.cnic
    );
  };

  // Check if toggle can be enabled based on mandatory fields and existing user selection
  // Around line 131
const toggleAddButton = (record) => {
  if (!record) return; // **Add this line to handle null record**
  
  if (isExistingUserSelected) {
    message.error("You cannot enable the toggle for an existing user. Please modify data or add a new seller.");
    return;
  }

  if (!areMandatoryFieldsFilled()) {
    message.error("Please fill all mandatory fields before enabling the toggle.");
    return;
  }

  setAddedSellers((prev) => {
    const updatedSellers = prev.some(seller => seller.id === record.sellerId)
      ? prev.filter(seller => seller.id !== record.sellerId)
      : [...prev, { 
          id: record.sellerId, 
          name: record.seller.name, 
          cnic: record.seller.cnic,
          phone: record.seller.phone,
          address: record.seller.address,
          image: record.seller.image,
          cnicImage1: record.seller.cnicImage1,
          cnicImage2: record.seller.cnicImage2
        }];

    localStorage.setItem('addedSellers', JSON.stringify(updatedSellers));
  ...
  const handleExistingUserSelect = (seller) => {
    sellerForm.setFieldsValue({
        _id: seller.id,  // Save the seller ID to associate products
        name: seller.name,
        phone: seller.phone,
    ...
    setIsExistingUserSelected(true); // Mark as existing user selected to restrict toggle
};


  // Delete an item from dropdown and update toggle state
  const deleteSellerFromDropdown = (sellerId) => {
    const updatedSellers = addedSellers.filter(seller => seller.id !== sellerId);
    setAddedSellers(updatedSellers);
    localStorage.setItem('addedSellers', JSON.stringify(updatedSellers));
  };

  // Filter addedSellers based on the search query
  const filteredSellers = addedSellers.filter(seller =>
    seller.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
    seller.cnic.includes(searchQuery)
  );
...
const columns = [
  ...
  // Dropdown menu items with delete icon on the left
  // Replace `menu` around line 174
const menuItems = [
  {
    key: 'search',
    label: (
  ...
  ...filteredSellers.map((seller) => ({
    ...
    icon: <DeleteOutlined
      onClick={(e) => {
        e.stopPropagation(); // Prevents triggering the select action
        deleteSellerFromDropdown(seller.id);
      }}
      className="delete-icon"
    />,
    label: <span onClick={() => handleExistingUserSelect(seller)}>{seller.name} - {seller.cnic}</span>,
  })),
];
...
<Table dataSource={filteredProducts} columns={columns} bordered />
  ...
  {categories.map((category) => (
    <Select.Option key={category} value={category}>{category}</Select.Option>
                  ...
                  <Col span={12}>
                    <FormItem name="quantity" label="Quantity" rules={[{ required: true }]}>
            ...
            <div className="saler-form" style={{ position: 'relative' }}>
              ...
              <Dropdown overlay={menu} trigger={['hover']} placement="bottomLeft">
                <button className="seller-form-existing-user">
                  Existing User <DownOutlined />
                </button>
              </Dropdown>
              ...
              <Form form={sellerForm} layout='vertical'>
                ...
                <FormItem name="image" label="Image Link">
                  <Input />
                </FormItem>
                <FormItem name="cnic" label="CNIC Number">
                  <Input />
                </FormItem>
                <Row gutter={16}>
                  <Col span={12}>
                    <FormItem name="cnicImage1" label="CNIC Image 1">
                      <Input />
                    </FormItem>
                  </Col>
                  <Col span={12}>
                    <FormItem name="cnicImage2" label="CNIC Image 2">
                      <Input />
                    </FormItem>
                  </Col>
                </Row>
              ...
              {/* Toggle Add Button */}
              <div className="seller-form-add-toggle">
                <Switch 
                  onChange={() => toggleAddButton(editProduct)} 
                  checked={addedSellers.some(seller => seller.id === editProduct?.sellerId)} 
